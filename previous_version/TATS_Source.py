...
class TatsProject(object):
    ...
    def identifier_referencing_function(
            self, name, function_name, line_number, file_name):
        pass

    def identifier_referencing_method(
    ...
    def analyze_project(self, project_file=''):
        ...
        for file_name in scripts_in_project:
            with open(file_name, 'r') as file:
                file_content_lines = self.prepare_file(file_name)
                global_file_content = self.parse_classes(
                        file_content_lines, file_name)
                global_file_content = self.parse_functions(
                        global_file_content, file_name)
                self.parse_variables(global_file_content, file_name, 1)
